...
class LtrRecordCodecTest :
    FunSpec({
        ...
        test("decode rejects an unknown proofType, including the reserved Lightning value 2") {
            val payer = Secp256k1KeyPair.generate()
            val viewId = Secp256k1KeyPair.generate().publicKey
            val bytes = LtrRecordCodec.encode(LtrRecord.create(payer, testCid(1), viewId, 1000, testProof()))
            ...
            val reservedLightning = bytes.copyOf()
            reservedLightning[proofTypeOffset] = 2
            shouldThrow<MalformedLtrRecordException> { LtrRecordCodec.decode(reservedLightning) }
